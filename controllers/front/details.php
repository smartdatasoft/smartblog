...
class  smartblogDetailsModuleFrontController extends smartblogModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        $comment = $blogcomment->getComments($id_post);
        $countcomment = $blogcomment->getTotalPosts($id_post);
        $id_cate = $post['id_category'];
        $title_category = $BlogCategory->getCategoryName($id_cate);
        ...
        if (Tools::isSubmit('addComment')) {
            ...
            if ($post['comment_status'] == 1) {
                ...
                if (empty($name)) {
                    ...
                    if (Configuration::get('smartacceptcomment') != '' && Configuration::get(
                            'smartacceptcomment'
                        ) != null
                    ) {
                        $value = Configuration::get('smartacceptcomment');
