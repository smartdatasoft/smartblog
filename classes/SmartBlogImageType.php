...
class SmartBlogImageType extends ObjectModel
{
    ...
    public static function ImageGenerate()
    {
        ...
        $get_cate_image = SmartBlogCategory::getCategoryImage();
        $get_author_image = _PS_MODULE_DIR_.'smartblog/images/avatar/avatar.jpg';
        $category_types = SmartBlogImageType::GetImageAllType('Category');
        ...
        foreach ($category_types as $image_type) {
            foreach ($get_cate_image as $cat_img) {
                ...
                ImageManager::resize(
                    $path,
                    _PS_MODULE_DIR_.'smartblog/images/category/'.$cat_img['id_smart_blog_category'].'-'.stripslashes(
        ...
        foreach ($posts_types as $image_type) {
            foreach ($get_blog_image as $blog_img) {
                ...
                ImageManager::resize(
                    $path,
                    _PS_MODULE_DIR_.'smartblog/images/'.$blog_img['id_smart_blog_post'].'-'.stripslashes(
                        $image_type['type_name']
                    ).'.jpg',
                    (int) $image_type['width'],
                    (int) $image_type['height']
        ...
        foreach ($author_types as $author_type) {
            ImageManager::resize(
                $get_author_image,
                _PS_MODULE_DIR_.'smartblog/images/avatar/avatar-'.stripslashes($author_type['type_name']).'.jpg',
                (int) $author_type['width'],
                (int) $author_type['height']
            );
        }
    }
    ...
    public static function ImageDelete()
    {
        ...
        foreach ($category_types as $image_type) {
            foreach ($get_cate_image as $cat_img) {
                ...
                $dir = _PS_MODULE_DIR_.'smartblog/images/category/'.$cat_img['id_smart_blog_category'].'-'.stripslashes(
                        $image_type['type_name']
                    ).'.jpg';
                if (file_exists($dir)) {
                    unlink($dir);
        ...
        foreach ($posts_types as $image_type) {
            foreach ($get_blog_image as $blog_img) {
                ...
                $dir = _PS_MODULE_DIR_.'smartblog/images/'.$blog_img['id_smart_blog_post'].'-'.stripslashes(
                        $image_type['type_name']
                    ).'.jpg';
                if (file_exists($dir)) {
                    unlink($dir);
