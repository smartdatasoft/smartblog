...
require_once(dirname(__FILE__).'/classes/SmartBlogCategory.php');
require_once(dirname(__FILE__).'/classes/SmartBlogcomment.php');
require_once(dirname(__FILE__).'/classes/SmartBlogPostCategory.php');
require_once(dirname(__FILE__).'/classes/SmartBlogTag.php');
require_once(dirname(__FILE__).'/classes/SmartBlogPost.php');
require_once(dirname(__FILE__).'/classes/SmartBlogImageType.php');
...
class smartblog extends Module
{
    public function __construct()
    {
        ...
        $this->description = $this->l('The Most Powerful PrestaShop Blog Module - by smartdatasoft');
    ...
    public function install()
    {
        ...
        Configuration::updateGlobalValue(
            'smartblogmetakeyword',
            'smart,blog,smartblog,prestashop blog,prestashop,blog'
        );
        Configuration::updateGlobalValue(
            'smartblogmetadescrip',
            'Prestashop powerfull blog site developing module. It has hundrade of extra plugins. This module developed by SmartDataSoft.com'
        );
        $this->addquickaccess();
...
<script type="text/javascript">
        $.post(
            ...
            if (Tools::getvalue('isdeleteoldthumblr') != 1) {
                SmartBlogImageType::ImageGenerate();
...
<script type="text/javascript">
        $.post(
            "ajax-tab.php",
            {
                ...
                SmartBlogImageType::ImageDelete();
                SmartBlogImageType::ImageGenerate();
...
<script type="text/javascript">
        ...
        if (isset($FILES['avatar']) && isset($FILES['avatar']['tmp_name']) && !empty($FILES['avatar']['tmp_name'])) {
            if ($error = ImageManager::validateUpload($FILES['avatar'], 4000000)) {
                ...
                if (!move_uploaded_file($FILES['avatar']['tmp_name'], $path)) {
                    ...
                    $author_types = SmartBlogImageType::GetImageAllType('author');
                    foreach ($author_types as $image_type) {
                        $dir = _PS_MODULE_DIR_.'smartblog/images/avatar/avatar-'.stripslashes(
                    ...
                    $images_types = SmartBlogImageType::GetImageAllType('author');
                    foreach ($images_types as $image_type) {
                        ImageManager::resize(
                            $path,
                            _PS_MODULE_DIR_.'smartblog/images/avatar/avatar-'.stripslashes(
                                $image_type['type_name']
                            ).'.jpg',
    ...
    public function SampleDataInstall()
    {
        ...
        Db::getInstance()->insert(
            'smart_blog_category_shop',
            array(
                'id_smart_blog_category' => 1,
                'id_shop' => $this->smart_shop_id,
            )
        );
        foreach (Language::getLanguages(false) as $language) {
            Db::getInstance()->insert(
                'smart_blog_category_lang',
                array(
                    'id_smart_blog_category' => 1,
                    'meta_title' => 'Uncategorized',
                    'id_lang' => (int) $language['id_lang'],
                    'link_rewrite' => 'uncategorized',
                )
            );
        }
        for ($i = 1; $i <= 4; $i++) {
                    ...
                    '
                       INSERT INTO `'._DB_PREFIX_.'smart_blog_post_lang`(`id_smart_blog_post`,`id_lang`,`meta_title`,`meta_description`,`short_description`,`content`,`link_rewrite`)
                        VALUES('.$i.','.(int) $language['id_lang'].', 
							...
							"'.htmlspecialchars($des).'","'.substr($des, 0, 200).'","'.htmlspecialchars(
                        $des
                    ).'","'.$slug.'"
						)'
                )
    ...
    public static function GetSmartBlogUrl()
    {
        ...
        if ((!$rewrite_set && in_array(
                    $id_shop,
                    array((int) Context::getContext()->shop->id, null)
                )) || !Language::isMultiLanguageActivated($id_shop) || !(int) Configuration::get(
